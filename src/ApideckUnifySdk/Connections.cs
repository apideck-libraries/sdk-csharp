...
namespace ApideckUnifySdk
{
    ...
    public interface IConnections
    {
        ...
        /// </summary>
        /// <remarks>
        /// Migrate the connection to the target connector, keeping its credentials and connection state<br/>
        /// (settings, metadata, configuration, subscriptions, consents). The source connection record is<br/>
        /// removed WITHOUT revoking or disconnecting the downstream token.<br/>
        /// <br/>
        /// Available migration targets are declared per connector — refer to the connector's<br/>
        /// documentation page or the Connector API's `migration_targets` field.<br/>
        /// <br/>
        /// Migrated tokens carry the source connector's OAuth scopes, so operations exclusive to the<br/>
        /// target connector may require re-authorization.<br/>
        /// <br/>
        /// Retries are idempotent: a partially-completed migration resumes where it left off.
        /// </remarks>
        /// <param name="request">A <see cref="VaultConnectionsMigrateRequest"/> parameter.</param>
        /// <param name="retryConfig">The retry configuration to use for this operation.</param>
        /// <returns>An awaitable task that returns a <see cref="VaultConnectionsMigrateResponse"/> response envelope when completed.</returns>
        /// <exception cref="ArgumentNullException">The required parameter <paramref name="request"/> is null.</exception>
        /// <exception cref="HttpRequestException">The HTTP request failed due to network issues.</exception>
        /// <exception cref="ResponseValidationException">The response body could not be deserialized.</exception>
        /// <exception cref="BadRequestResponse">Bad Request. Thrown when the API returns a 400 response.</exception>
        ...
        /// <exception cref="PaymentRequiredResponse">Payment Required. Thrown when the API returns a 402 response.</exception>
        /// <exception cref="NotFoundResponse">The specified resource was not found. Thrown when the API returns a 404 response.</exception>
        /// <exception cref="ConflictResponse">Conflict — a connection already exists for the target connector. Thrown when the API returns a 409 response.</exception>
        /// <exception cref="UnprocessableResponse">Unprocessable. Thrown when the API returns a 422 response.</exception>
        /// <exception cref="APIException">Default API Exception. Thrown when the API returns a 4XX or 5XX response.</exception>
        public  Task<VaultConnectionsMigrateResponse> MigrateAsync(
            VaultConnectionsMigrateRequest request,
            RetryConfig? retryConfig = null
        );
    ...
    public class Connections: IConnections
    {
        ...
        /// <exception cref="UnprocessableResponse">Unprocessable. Thrown when the API returns a 422 response.</exception>
        /// <exception cref="APIException">Default API Exception. Thrown when the API returns a 4XX or 5XX response.</exception>
        public async  Task<VaultConnectionsMigrateResponse> MigrateAsync(
            VaultConnectionsMigrateRequest request,
            RetryConfig? retryConfig = null
        )
        {
            ...
            string baseUrl = this.SDKConfiguration.GetTemplatedServerUrl();
            var urlString = URLBuilder.Build(baseUrl, "/vault/connections/{unified_api}/{service_id}/migrate", request, null);
            ...
            var serializedBody = RequestBodySerializer.Serialize(request, "ConnectionMigrateData", "json", false, false);
            if (serializedBody != null)
            {
                httpRequest.Content = serializedBody;
            }
            ...
            var hookCtx = new HookContext(SDKConfiguration, baseUrl, "vault.connectionsMigrate", null, SDKConfiguration.SecuritySource);
            ...
            if(responseStatusCode == 200)
            {
                if(Utilities.IsContentTypeMatch("application/json", contentType))
                {
                    ...
                    var response = new VaultConnectionsMigrateResponse()
                    {
                        HttpMeta = new Models.Components.HTTPMetadata()
                        {
                            Response = httpResponse,
            ...
            else if(responseStatusCode == 409)
            {
                ...
                {
                    var httpResponseBody = await httpResponse.Content.ReadAsStringAsync();
                    ConflictResponsePayload payload;
                    try
                    {
                        payload = ResponseBodyDeserializer.DeserializeNotNull<ConflictResponsePayload>(httpResponseBody, NullValueHandling.Ignore);
                    }
                    catch (Exception ex)
                    {
                        throw new ResponseValidationException("Failed to deserialize response body into ConflictResponsePayload.", httpRequest, httpResponse, httpResponseBody, ex);
                    }

                    throw new ConflictResponse(payload, httpRequest, httpResponse, httpResponseBody);
                }
