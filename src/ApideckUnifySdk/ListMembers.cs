...
namespace ApideckUnifySdk
{
    ...
    public interface IListMembers
    {
        ...
        /// </remarks>
        /// <param name="request">A <see cref="CrmListMembersAllRequest"/> parameter.</param>
        /// <param name="retryConfig">The retry configuration to use for this operation.</param>
        /// <returns>An awaitable task that returns a <see cref="CrmListMembersAllResponse"/> response envelope when completed.</returns>
        ...
        public  Task<CrmListMembersAllResponse> ListAsync(
            CrmListMembersAllRequest request,
    ...
    public class ListMembers: IListMembers
    {
        ...
        public ListMembers(SDKConfig config)
        {
        ...
        /// <exception cref="UnprocessableResponse">Unprocessable. Thrown when the API returns a 422 response.</exception>
        /// <exception cref="APIException">Default API Exception. Thrown when the API returns a 4XX or 5XX response.</exception>
        public async  Task<CrmListMembersAllResponse> ListAsync(
            CrmListMembersAllRequest request,
            RetryConfig? retryConfig = null
        )
        {
            ...
            var urlString = URLBuilder.Build(baseUrl, "/crm/lists/{id}/members", request, null);
            ...
            var hookCtx = new HookContext(SDKConfiguration, baseUrl, "crm.listMembersAll", null, SDKConfiguration.SecuritySource);
            ...
            Func<Task<CrmListMembersAllResponse?>> nextFunc = async delegate()
            {
                ...
                var newRequest = new CrmListMembersAllRequest
                {
                    Id = request.Id,
                    Raw = request.Raw,
            ...
            if(responseStatusCode == 200)
            {
                if(Utilities.IsContentTypeMatch("application/json", contentType))
                {
                    ...
                    GetListMembersResponse obj;
                    try
                    {
                        obj = ResponseBodyDeserializer.DeserializeNotNull<GetListMembersResponse>(httpResponseBody, NullValueHandling.Ignore);
                    }
                    catch (Exception ex)
                    {
                        throw new ResponseValidationException("Failed to deserialize response body into GetListMembersResponse.", httpRequest, httpResponse, httpResponseBody, ex);
                    }

                    var response = new CrmListMembersAllResponse()
                    {
                    ...
                    response.GetListMembersResponse = obj;
                    return response;
                }
