...
namespace ApideckUnifySdk
{
    ...
    public interface IConnectionConsent
    {
        /// <summary>
        ...
        /// </remarks>
        /// <param name="request">A <see cref="VaultConnectionConsentUpdateRequest"/> parameter.</param>
        /// <param name="retryConfig">The retry configuration to use for this operation.</param>
        /// <returns>An awaitable task that returns a <see cref="VaultConnectionConsentUpdateResponse"/> response envelope when completed.</returns>
        ...
        public  Task<VaultConnectionConsentUpdateResponse> UpdateAsync(
            VaultConnectionConsentUpdateRequest request,
            RetryConfig? retryConfig = null
        );
    }

    public class ConnectionConsent: IConnectionConsent
    {
        /// <summary>
        /// SDK Configuration.
        /// <see cref="SDKConfig"/>
        ...
        public ConnectionConsent(SDKConfig config)
        {
        ...
        /// <exception cref="UnprocessableResponse">Unprocessable. Thrown when the API returns a 422 response.</exception>
        /// <exception cref="APIException">Default API Exception. Thrown when the API returns a 4XX or 5XX response.</exception>
        public async  Task<VaultConnectionConsentUpdateResponse> UpdateAsync(
            VaultConnectionConsentUpdateRequest request,
            RetryConfig? retryConfig = null
        )
        {
            ...
            var urlString = URLBuilder.Build(baseUrl, "/vault/connections/{unified_api}/{service_id}/consent", request, null);

            var httpRequest = new HttpRequestMessage(HttpMethod.Patch, urlString);
            httpRequest.Headers.Add("user-agent", SDKConfiguration.UserAgent);
            HeaderSerializer.PopulateHeaders(ref httpRequest, request);

            var serializedBody = RequestBodySerializer.Serialize(request, "UpdateConsentRequest", "json", false, false);
            ...
            var hookCtx = new HookContext(SDKConfiguration, baseUrl, "vault.connectionConsentUpdate", null, SDKConfiguration.SecuritySource);
            ...
            if(responseStatusCode == 200)
            {
                if(Utilities.IsContentTypeMatch("application/json", contentType))
                {
                    ...
                    UpdateConsentResponse obj;
                    try
                    {
                        obj = ResponseBodyDeserializer.DeserializeNotNull<UpdateConsentResponse>(httpResponseBody, NullValueHandling.Ignore);
                    }
                    catch (Exception ex)
                    {
                        throw new ResponseValidationException("Failed to deserialize response body into UpdateConsentResponse.", httpRequest, httpResponse, httpResponseBody, ex);
                    }

                    var response = new VaultConnectionConsentUpdateResponse()
                    {
                    ...
                    response.UpdateConsentResponse = obj;
                    return response;
                }
