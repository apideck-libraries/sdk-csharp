...
namespace ApideckUnifySdk
{
    ...
    public interface IConnectorDocs
    {
        /// <summary>
        ...
        /// <returns>An awaitable task that returns a <see cref="ConnectorConnectorDocsOneResponse"/> response envelope when completed.</returns>
        /// <exception cref="ArgumentNullException">One of <paramref name="id"/> or <paramref name="docId"/> is null.</exception>
        ...
        public  Task<ConnectorConnectorDocsOneResponse> GetAsync(
            string id,
            string docId,
            string? appId = null,
    ...
    public class ConnectorDocs: IConnectorDocs
    {
        /// <summary>
        /// SDK Configuration.
        /// <see cref="SDKConfig"/>
        ...
        public ConnectorDocs(SDKConfig config)
        {
        ...
        /// <exception cref="NotFoundResponse">The specified resource was not found. Thrown when the API returns a 404 response.</exception>
        /// <exception cref="APIException">Default API Exception. Thrown when the API returns a 4XX or 5XX response.</exception>
        public async  Task<ConnectorConnectorDocsOneResponse> GetAsync(
            string id,
            string docId,
            string? appId = null,
            RetryConfig? retryConfig = null
        )
        {
            ...
            var urlString = URLBuilder.Build(baseUrl, "/connector/connectors/{id}/docs/{doc_id}", request, null);
            ...
            {
                httpRequest.Headers.Add("Accept", "application/json;q=1, text/markdown;q=0");
            }
            ...
            var hookCtx = new HookContext(SDKConfiguration, baseUrl, "connector.connectorDocsOne", null, SDKConfiguration.SecuritySource);
            ...
            {
                if(Utilities.IsContentTypeMatch("text/markdown", contentType))
                {
                    var response = new ConnectorConnectorDocsOneResponse()
                    {
                    ...
                    response.GetConnectorDocResponse = await httpResponse.Content.ReadAsStringAsync();
                    return response;
                }
