...
Developer-friendly & type-safe Csharp SDK specifically catered to leverage *Apideck.Unify.SDK* API.
...
## Table of Contents
<!-- $toc-max-depth=2 -->
* [Apideck.Unify.SDK](#apideckunifysdk)
...
```bash
dotnet add reference src/Apideck/Unify/SDK/Apideck.Unify.SDK.csproj
```
...
By default, an API error will raise a `Apideck.Unify.SDK.Models.Errors.APIException` exception, which has the following properties:
...
catch (Exception ex)
{
    ...
    else if (ex is Apideck.Unify.SDK.Models.Errors.APIException)
    {
...
### SDK Created by [Speakeasy](https://www.speakeasy.com/?utm_source=apideck-unify-sdk&utm_campaign=csharp)
